...
question("Simon has been asked to find any outliers in each month in the same dataset, we class an outlier as any sale that has a price more than 3 standard deviations away from the mean (in either direction), or has a number of items more than 3 sd away from the mean (in either direction). Which code should he use?",
    answer("`df1 %>% group_by(month) %>% 
mutate(mn_tp = mean(transaction_price), sd_tp = sd(transaction_price),
mn_ni = mean(number_of_item), sd_ni = sd(number_of_item)) %>%
